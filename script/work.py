...
new_df = pd.DataFrame(columns=['流行语名称','总日均','总同比','总环比','PC端日均','PC端同比','PC端环比','移动端日均','移动端同比','移动端环比'])
count=0
for word in word.iloc[0:100]:
    print(f"{count}：",word)
    time.sleep(1)
    resp=requests.get(f"https://index.baidu.com/api/SearchApi/index?area=0&word=[[%7B%22name%22:%22{word}%22,%22wordType%22:1%7D]]&&startDate={2022}-01-01&&endDate={2022}-12-31",headers=headers)
    try:
        search_api_obj=json.loads(resp.content)['data']['generalRatio'][0]
        print(search_api_obj['all'],search_api_obj['pc'],search_api_obj['wise'])
        all=search_api_obj['all']
        pc=search_api_obj['pc']
        wise=search_api_obj['wise']
        ...
        count+=1
print(new_df)
new_df.to_excel("../data/2022-搜索指数.xlsx")